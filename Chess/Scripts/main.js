class Main {
    ...
    initialize(settings) {
        ...
        this.updateSwapSidesButtonState()
    ...
    setupElementSelectors() {
        ...
        this.$swapSidesButton = $("#swap-sides-button");
        this.$undoButton = $("#undo-button");
    }

    setupEventListeners() {
        $(window).on("resize", this.scaleBoard.bind(this));
        this.$menuIcon.on("click", () => this.$menuContainer.toggleClass("open"));
        this.$okButton.on("click", () => this.$menuContainer.removeClass("open"));
        this.$menuContainer.on("click", (e) => {
            if (e.target === this.$menuContainer[0]) {
                this.$menuContainer.removeClass("open");
            }
        });
        this.$newGameButton.on("click", () => window.location.reload());
        $('input[name="color-scheme"]').on('change', (e) => this.setColorScheme(e.target.id.split('-')[0]));
        ...
        this.$highlightSwitch.on('change', (e) => this.setHighlighting(e.target.checked));
    ...
    updateSwapSidesButtonState() {
        this.$swapSidesButton.prop('disabled', this.current_player_types[0] === this.current_player_types[1]);
    }
